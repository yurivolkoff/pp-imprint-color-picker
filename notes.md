...
- 2026-05-20: **v4 — formatting + picker overlay** in response to Yuri review.
  - **Scrollbar shift fixed.** `scrollbar-gutter: stable` on `.modal-body`. Modal content no longer shifts horizontally when scrollbar would appear.
  - **Sections reordered** — Imprint colors → Artwork file → Artwork instructions. Color is the structured-data input the artwork team most depends on; file upload is optional and can come after the order.
  - **Artwork file section got a title** (was the only section without one).
  - **Typography system documented as CSS custom properties** (`--fs-section`, `--fw-section`, `--fc-section`, `--fs-sublabel`, etc.) and applied uniformly. All three section titles share the same style: 14 px / 700 / normal-case / `text-dark`. Sub-labels share 11 px / 700 / uppercase / `text-muted`. Help text consistent at 12 px / 400 / `text-muted`. No more 16 px h3 vs. 12 px label mismatch.
  - **Button system standardized**: `.btn-primary` (orange CTA), `.btn-secondary` (navy, picker Done), `.btn-outline` (Add color closed), `.btn-toggle` (Add color when picker open), `.btn-ghost` (Close, modal foot), `.btn-pill-sm` (PMS filters, demo buttons), `.btn-link` (foot links). All use `transition: all 0.15s ease` and consistent hover patterns within variant.
  - **Standard picker compactified** — label-left, swatches-right grid (`grid-template-columns: 120px 1fr`). Each family is a single row. Recently-used row removed. Tier-tabs padding tightened (7 px). All 7 standard families now fit on screen without scrolling.
  - **Picker is now an overlay** within modal body — when open it absolute-positions over the artwork sections instead of pushing them down. Sections behind the picker get `opacity: 0.18` via `.muted` class (effectively hidden behind opaque picker; visible at modal-foot level).
  - **Modal-body height fixed at 700 px** — same dimensions whether picker is open or closed. No more vertical jitter.
  - **Auto-scroll on open** — chip-zone position is computed via JS and the picker is anchored just below it. `modalBody.scrollTo` brings the picker-section to the top of the viewport when picker opens.
  - **JS positioning** — `positionPicker()` calculates picker top via `getBoundingClientRect` after every state change that could move the chip-zone, then re-anchors the absolute picker. Re-runs on window resize.
